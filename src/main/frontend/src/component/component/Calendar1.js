...
//    const doctorId = cookieValue.doctorId;
    ...
    return (
        <>
            <div className="calendar" style={{margin:"0px",float:"left"}}>
                <div className="dropdowns">
                    <select name="year" value={selectedDate.format('YYYY')} onChange={handleChange}>
                        {Array.from({length: 100}, (_, i) => moment().year() - i).map((year) => (
                            <option key={year} value={year}>
            ...
            <div className='tableC' style={{marginLeft:"100px", marginTop:"-30px",width:"400px", display:"inline-block"}}>
                {isTableVisible && clickedDate && (
                    (notReservation ? <div style={{marginTop:"30px",textAlign:"center", fontSize:"20px"}}>해당 날짜의<br/>예약 일정이 설정되지 않았습니다.</div> : <TableComponent1 selectedDate={clickedDate} reservationData={reservationData}
                                                               reservationList={reservationList} handleTimeClick={handleTimeClick}
