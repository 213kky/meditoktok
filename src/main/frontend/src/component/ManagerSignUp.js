    ...
    return (
        <section className="contents">
            ...
            <form onSubmit={handleSubmit} style={{marginLeft: "425px"}}>
                ...
                <div>이메일 수신 동의
                    <input type="checkbox" checked={mailAgree}
                           onChange={(event) => setMailAgree(event.target.checked)}/>
                </div>
                ...
                <div>
                    ...
                    {totalCount > 0 ? (totalCount === 1 ? <ul>
                                <li key={searchResults.ykiho} onClick={() => handleHospitalSelect(searchResults)}>
                                    <div>{searchResults.yadmNm}</div>
                                    <div>{searchResults.addr}</div>
                                    {/* 필요한 데이터들 추가 */}
                                </li>
                            </ul> :
                            <ul>
